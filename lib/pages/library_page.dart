...
class _LibraryPageState extends State<LibraryPage> {
  ...
  SortOption? _cachedSortOption;
  List<Song>? _cachedSongs;

  List<Song> get _filteredSongs {
    // 检查是否需要重新计算
    final needsUpdate = _cachedSongs != widget.songs ||
                       _cachedSearchQuery != _searchQuery ||
                       _cachedSortOption != _sortOption;
    
    if (!needsUpdate && _cachedFilteredSongs != null) {
    ...
    // 更新缓存
    _cachedSongs = widget.songs;
    _cachedSearchQuery = _searchQuery;
    _cachedSortOption = _sortOption;
    _cachedFilteredSongs = songs;
  ...
  Future<void> _scanMusic() async {
    ...
    try {
      ...
      // 扫描音乐文件（增量扫描）
      final scannedSongs = await widget.scanner.scanMusicFiles();

      if (mounted) {
        widget.onSongsUpdated(scannedSongs);
        
        if (scannedSongs.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('未找到音乐文件')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('找到 ${scannedSongs.length} 首歌曲')),
          );
        }
      }
    } catch (e) {
